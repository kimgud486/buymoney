...
class EnhancedSignalEngineTests(unittest.TestCase):
    ...
    def test_costs_are_subtracted_from_pattern_ev(self):
        ...
        pattern = SimpleNamespace(
            ...
            profit_score=55.0,
            similarity=60.0,
            success_similarity=62.0,
            failure_similarity=58.0,
            avg_forward_return=0.15,
        )
        cfg = EnhancedSignalConfig(
            round_trip_fee_bps=10.0,
            slippage_bps=10.0,
            max_vwap_extension_pct=100.0,
            max_atr_extension=100.0,
        )
    ...
    def test_small_pattern_sample_is_shrunk_toward_fifty(self):
        ...
        pattern = SimpleNamespace(
            ...
            profit_score=70.0,
            similarity=75.0,
            success_similarity=80.0,
            failure_similarity=50.0,
            avg_forward_return=0.8,
        )
        cfg = EnhancedSignalConfig(
            probability_prior_strength=25.0,
            max_vwap_extension_pct=100.0,
            max_atr_extension=100.0,
        )
        result = analyze_signal_quality(rows, pattern, cfg)
