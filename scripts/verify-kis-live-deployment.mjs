...
if (!rawBaseUrl) {
  fail("BUYMONEY_LIVE_URL is required. Example: https://your-service.run.app");
}
...
if (!/^\d{6}$/.test(symbol)) {
  fail("BUYMONEY_PROOF_SYMBOL must be a 6-digit Korean stock code.");
}
...
for (const [label, value] of [["quoteAgeMs", payload?.quoteAgeMs], ["accountAgeMs", payload?.accountAgeMs]]) {
  if (!Number.isFinite(Number(value)) || Number(value) < 0 || Number(value) > 30_000) {
    fail(`${label} must be between 0 and 30000ms.`, value);
  }
}
...
console.log(JSON.stringify({
  ...
  accountAgeMs: payload.accountAgeMs,
  proofStatus: proof.status,
  checkedAt: proof.checkedAt,
  automaticOrderSubmission: payload.canSubmitOrder,
}, null, 2));
