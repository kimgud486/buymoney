...
const DEFAULT_INTRADAY_COUNT = 180;
...
class ServerCandleWarmCoordinatorV20 {
  ...
  private async warmOne(request: CandleWarmRequestV20): Promise<boolean> {
    ...
    const intradayCount = request.market === "KOREA" ? KOREA_INTRADAY_COUNT : DEFAULT_INTRADAY_COUNT;

    const [oneMinuteResponse, fifteenMinuteResponse] = await Promise.all([
      fetch(`${base}/api/market/realtime-candles?symbol=${symbol}&timeframe=1m&count=${intradayCount}`, { signal: AbortSignal.timeout(12000) }),
    ...
    let derived = false;
    if (fifteenMinute.length < MIN_15M_ANALYSIS && request.market === "KOREA") {
