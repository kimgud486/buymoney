...
type MarketFilter = "ALL" | ScanMarket;
...
function serverMarket(filter: MarketFilter): string {
  return filter === "BTC" ? "UPBIT" : filter;
}

function marketLabel(filter: MarketFilter): string {
  if (filter === "KOREA") return "🇰🇷 국내";
  if (filter === "US") return "🇺🇸 해외";
  if (filter === "BTC") return "🪙 업비트";
  return "🌐 전체";
}
...
function shortScoreFrom(result: VerifiedSignalResult): number {
  ...
  if (!m.hhhl) score += 8;
  score += Math.min(15, result.patternHits.filter((hit) => hit.direction === "BEARISH").reduce((sum, hit) => sum + Math.max(2, hit.weight), 0));
...
function chooseDirection(frames: TimeframeScan[]): { direction: TradeDirection; longScore: number; shortScore: number } {
  ...
  const weights: Record<TimeframeKey, number> = { "1m": 0.6, "3m": 0.8, "5m": 1.2, "15m": 1.4, "30m": 1.4, "60m": 1.7, D: 1.9 };
  let bullish = 0;
  let bearish = 0;
  let total = 0;
  ...
  const longFrameScore = total ? (bullish / total) * 100 : 0;
  const shortFrameScore = total ? (bearish / total) * 100 : 0;
...
async function fetchServerDiscovery(filter: MarketFilter): Promise<ServerDiscovery> {
  const response = await fetch(`/api/explainable-scanner?market=${encodeURIComponent(serverMarket(filter))}&aiExplain=true`, { cache: "no-store" });
  ...
  const candidates = rows.flatMap((idea: any): UniverseItem[] => {
    ...
    const market = marketFromRaw(symbol, idea?.market);
    if (filter !== "ALL" && market !== filter) return [];
    return [{
      symbol,
...
async function fetchOneFrame(symbol: string, tf: typeof TIMEFRAMES[number]): Promise<{ payload: any; result: VerifiedSignalResult } | null> {
  try {
    const response = await fetch(`/api/market/realtime-candles?symbol=${encodeURIComponent(symbol)}&timeframe=${encodeURIComponent(tf.api)}&count=70`, { cache: "no-store" });
...
async function fetchVerifiedCandidate(base: UniverseItem): Promise<ScannedStockItem | null> {
  ...
  const scores = chooseDirection(timeframes);
  if (scores.direction === "WAIT" && Math.max(scores.longScore, scores.shortScore) < 62) return null;
  const anchor = valid.find((v) => v.timeframe === "5m") || valid.find((v) => v.timeframe === "15m") || valid[0];
  const currentPrice = Number(anchor.payload?.currentPrice) > 0 ? Number(anchor.payload.currentPrice) : base.price;
  if (!(currentPrice > 0)) return null;
  const atr = Math.max(anchor.result.metrics.atr, currentPrice * 0.008);
  const short = scores.direction === "SHORT";
  ...
  const aiScore = scores.direction === "SHORT" ? scores.shortScore : scores.longScore;
  const mainPattern = timeframes.filter((f) => f.pattern !== "NONE").sort((a, b) => b.score - a.score)[0];
  const alignedFrames = timeframes.filter((f) => scores.direction === "LONG" ? f.direction === "BULLISH" : scores.direction === "SHORT" ? f.direction === "BEARISH" : false);
  return {
    id: `${base.market}_${base.symbol}`,
    symbol: base.symbol,
    name: anchor.payload?.name || base.name,
    market: base.market,
    currentPrice,
    changePct: Number.isFinite(Number(anchor.payload?.changePct)) ? Number(anchor.payload.changePct) : base.changePct,
    ...
    decision: scores.direction === "LONG" && aiScore >= 82 ? "BUY_APPROVED" : "BUY_WATCH",
    ...
    rationale: `${alignedFrames.length}개 시간봉 방향 일치`,
    reasons: [`${alignedFrames.length}개 시간봉에서 ${scores.direction === "LONG" ? "위로 가는 힘" : scores.direction === "SHORT" ? "아래로 가는 힘" : "방향 대기"}을 확인했어요.`, ...(anchor.result.reasons || []).slice(0, 3).map(easyReason)],
...
async function refreshFastPatternFrames(item: ScannedStockItem): Promise<ScannedStockItem | null> {
  ...
  return {
    ...
    stopLoss: short ? currentPrice + atr * 1.2 : anchor.result.stopLoss,
    targetPrice: short ? Math.max(0, currentPrice - atr * 2) : anchor.result.target1,
    targetPrice2: short ? Math.max(0, currentPrice - atr * 3) : anchor.result.target2,
    reasons: [`1·3·5분봉 중 ${alignedFast}개가 지금 ${direction === "LONG" ? "상승" : direction === "SHORT" ? "하락" : "대기"} 방향이에요.`, ...(anchor.result.reasons || []).slice(0, 3).map(easyReason)],
    failedChecks: anchor.result.failedChecks,
    scannedAt: new Date().toLocaleTimeString("ko-KR", { hour: "2-digit", minute: "2-digit", second: "2-digit" }),
    verifiedBars: anchor.result.evaluatedBars,
...
export const RealtimeScannerTileBoard: React.FC<RealtimeScannerTileBoardProps> = ({ onSelectStock, isWhiteTheme = false }) => {
  ...
  const [selectedMarketFilter, setSelectedMarketFilter] = useState<MarketFilter>("ALL");
  ...
  const runVerifiedScan = useCallback(async () => {
    ...
    const activeFilter = selectedMarketFilter;
    setIsLoading(true);
    setScanError(null);
    setScanProgress({ done: 0, total: 0 });
    fastDirtySymbolsRef.current.clear();
    try {
      const discovery = await fetchServerDiscovery(activeFilter);
      if (requestId !== scanRequestIdRef.current) return;
      setServerScannedTotal(discovery.totalScanned);
      setScanProgress({ done: 0, total: discovery.candidates.length });
      const verified: ScannedStockItem[] = [];
      for (let offset = 0; offset < discovery.candidates.length; offset += BATCH_SIZE) {
        const batch = discovery.candidates.slice(offset, offset + BATCH_SIZE);
        ...
        setScanProgress({ done: Math.min(offset + batch.length, discovery.candidates.length), total: discovery.candidates.length });
      }
      ...
      if (requestId !== scanRequestIdRef.current) return;
      console.warn("[MergedScanner] market-specific discovery failed", error);
      setStocks([]);
      setServerScannedTotal(0);
      setScanError(`${marketLabel(activeFilter)} 스캔 결과를 확인하지 못했어요. 가짜 후보는 만들지 않습니다.`);
    } finally {
      if (requestId === scanRequestIdRef.current) setIsLoading(false);
    }
  }, [selectedMarketFilter]);

  useEffect(() => { void runVerifiedScan(); }, [runVerifiedScan]);
  useEffect(() => {
    if (!isAutoScanActive) return;
    const timer = window.setInterval(() => { void runVerifiedScan(); }, 60_000);
  ...
  const displayedStocks = useMemo(() => stocks, [stocks]);
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold text-cyan-400">{marketLabel(selectedMarketFilter)}</span>
            ...
            <div className="text-[10px] text-slate-400">선택시장 서버스캔 {serverScannedTotal.toLocaleString()}종목 · 가격 틱 즉시 · 포착종목 1/3/5분 패턴 1초 재평가 / 마지막 {lastScanAt}</div>
          </div>
        </div>
        <div className="flex items-center gap-1.5">
          {(["ALL", "KOREA", "US", "BTC"] as const).map((market) => (
            <button
              ...
              disabled={isLoading && selectedMarketFilter === market}
              className={`px-2 py-1 rounded text-[10px] font-bold ${selectedMarketFilter === market ? "bg-cyan-600 text-white" : "bg-slate-500/10 text-slate-400"}`}
              title={`${marketLabel(market)}만 서버에서 다시 스캔`}
            >
              {marketLabel(market)}
            </button>
          ))}
          <button type="button" onClick={() => setIsAutoScanActive((v) => !v)} className={`px-2 py-1 rounded text-[10px] font-bold ${isAutoScanActive ? "bg-emerald-500/15 text-emerald-400" : "bg-slate-500/10 text-slate-400"}`}>{isAutoScanActive ? "자동스캔 ON" : "자동스캔 OFF"}</button>
          <button type="button" onClick={() => void runVerifiedScan()} disabled={isLoading} className="p-1.5 rounded bg-cyan-500/10 text-cyan-400" title="선택한 시장 지금 다시 스캔"><RefreshCw className={`h-3.5 w-3.5 ${isLoading ? "animate-spin" : ""}`} /></button>
          <button type="button" onClick={() => setIsCollapsed((v) => !v)} className="p-1.5 text-slate-400">{isCollapsed ? <ChevronDown className="h-4 w-4" /> : <ChevronUp className="h-4 w-4" />}</button>
        </div>
      </div>

      {!isCollapsed && (
        <div className="p-2.5">
          {isLoading && <div className="mb-2 rounded-lg border border-cyan-500/20 bg-cyan-500/5 px-3 py-2 text-[11px] text-cyan-400">{marketLabel(selectedMarketFilter)}만 서버에서 스캔 중이에요. {scanProgress.total ? `${scanProgress.done}/${scanProgress.total}` : "PRECHECK 중"}</div>}
          ...
          {!isLoading && displayedStocks.length === 0 ? (
            <div className="py-8 text-center text-xs text-slate-400">{marketLabel(selectedMarketFilter)}에서 지금 조건에 맞는 종목이 없어요. 다른 시장 종목으로 채우지 않습니다.</div>
          ) : (
                    ...
                    {expanded && (
                      <div className={`border-t p-3 ${isWhiteTheme ? "border-slate-200 bg-slate-50/70" : "border-slate-700/40 bg-black/10"}`}>
                        <div className="grid gap-2 md:grid-cols-2">
                          <div className="rounded-lg bg-slate-500/10 p-2.5"><div className="text-xs font-black">🤖 왜 포착했나요?</div><div className="mt-2 space-y-1 text-[11px]">{item.reasons.slice(0, 4).map((reason, index) => <div key={index}>✓ {reason}</div>)}</div></div>
                          <div className="rounded-lg bg-slate-500/10 p-2.5"><div className="text-xs font-black">🎯 AI가 보고 있는 가격</div><div className="mt-2 grid grid-cols-2 gap-y-1 text-[11px]"><span className="text-slate-400">좋은 진입 자리</span><b>{item.entryZone}</b><span className="text-slate-400">위험 기준</span><b className="text-rose-400">{formatPlainPrice(item.stopLoss, item.market)}</b><span className="text-slate-400">1차 목표</span><b>{formatPlainPrice(item.targetPrice, item.market)}</b><span className="text-slate-400">2차 목표</span><b>{formatPlainPrice(item.targetPrice2, item.market)}</b></div></div>
                        </div>
                        <div className="mt-2 rounded-lg border border-slate-500/20 overflow-hidden">
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7">
                            {TIMEFRAMES.map((tf) => {
                              ...
                              return <div key={tf.key} className="border-t border-r border-slate-500/10 p-2 text-[10px]"><div className="font-black">{tf.key === "D" ? "일봉" : tf.key}</div><div className={frame?.direction === "BULLISH" ? "text-emerald-400" : frame?.direction === "BEARISH" ? "text-rose-400" : "text-slate-400"}>{frame?.direction === "BULLISH" ? "↗ 위쪽" : frame?.direction === "BEARISH" ? "↘ 아래쪽" : "- 대기"}</div><div className="mt-0.5 truncate text-slate-400" title={frame ? patternLabel(frame.pattern) : "데이터 없음"}>{frame ? patternLabel(frame.pattern) : "데이터 없음"}</div></div>;
                            })}
                          </div>
                        </div>
      ...
      {entryDialog && (
        <div className="fixed inset-0 z-[120] bg-black/65 flex items-center justify-center p-4" onMouseDown={() => setEntryDialog(null)}>
          <div className={`w-full max-w-md rounded-2xl border p-4 shadow-2xl ${isWhiteTheme ? "bg-white border-slate-200 text-slate-900" : "bg-[#081525] border-slate-700 text-white"}`} onMouseDown={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between gap-3"><div><div className="text-sm font-black">{entryDialog.direction === "LONG" ? "🟢 LONG" : "🔴 SHORT"} · {entryDialog.item.name}</div><div className="text-[10px] text-slate-400 mt-0.5">현재가 {formatPrice(entryDialog.item.currentPrice, entryDialog.item.market)}</div></div><button type="button" onClick={() => setEntryDialog(null)} className="p-1 text-slate-400"><X className="h-4 w-4" /></button></div>
            {entryDialog.step === "WHY" && <><div className="mt-4 rounded-xl bg-slate-500/10 p-3"><div className="font-black text-sm">왜 {entryDialog.direction}을 살펴보나요?</div><div className="mt-2 space-y-1.5 text-xs">{entryDialog.item.reasons.slice(0, 4).map((reason, index) => <div key={index}>✓ {reason}</div>)}<div>✓ 그래프 한 개만 보지 않고 여러 시간봉을 함께 봤어요.</div></div></div><div className="mt-3 text-[11px] text-slate-400">이 버튼은 아직 주문하지 않습니다. 설명을 읽고 다음 버튼을 눌러야 진입 확인 단계로 갑니다.</div><button type="button" onClick={() => setEntryDialog({ ...entryDialog, step: "CONFIRM" })} className={`mt-4 w-full py-3 rounded-xl font-black text-sm ${entryDialog.direction === "LONG" ? "bg-emerald-600 text-white" : "bg-rose-600 text-white"}`}>{entryDialog.direction} 진입 살펴보기</button></>}
            {entryDialog.step === "CONFIRM" && <><div className="mt-4 text-center text-lg font-black">{entryDialog.direction} 진입하시겠습니까?</div><div className="mt-3 rounded-xl bg-slate-500/10 p-3 grid grid-cols-2 gap-y-2 text-xs"><span className="text-slate-400">AI 진입구간</span><b>{entryDialog.item.entryZone}</b><span className="text-slate-400">위험 기준</span><b className="text-rose-400">{formatPlainPrice(entryDialog.item.stopLoss, entryDialog.item.market)}</b><span className="text-slate-400">1차 목표</span><b>{formatPlainPrice(entryDialog.item.targetPrice, entryDialog.item.market)}</b><span className="text-slate-400">2차 목표</span><b>{formatPlainPrice(entryDialog.item.targetPrice2, entryDialog.item.market)}</b></div><label className="block mt-3 text-xs font-bold">거래할 금액</label><input value={entryDialog.amount} onChange={(e) => setEntryDialog({ ...entryDialog, amount: e.target.value.replace(/[^0-9.]/g, "") })} inputMode="decimal" className={`mt-1 w-full rounded-lg border px-3 py-2 text-sm font-mono ${isWhiteTheme ? "bg-white border-slate-300" : "bg-slate-950 border-slate-700"}`} /><div className="mt-2 text-[11px] text-slate-400">확인을 눌러도 옛 가격으로 바로 주문하지 않아요. 5분봉과 현재가격을 다시 확인하고 좋은 자리일 때만 기존 주문 확인창으로 넘깁니다.</div>{entryDialog.direction === "SHORT" && entryDialog.item.market === "BTC" && <div className="mt-2 text-[11px] text-amber-400">업비트 현물은 SHORT 분석은 가능하지만 실제 SHORT 주문은 지원 상품이 따로 필요해요.</div>}<div className="mt-4 grid grid-cols-2 gap-2"><button type="button" onClick={() => setEntryDialog(null)} className="py-2.5 rounded-xl bg-slate-500/10 font-bold">취소</button><button type="button" onClick={recheckAndRequestOrder} className={`py-2.5 rounded-xl font-black text-white ${entryDialog.direction === "LONG" ? "bg-emerald-600" : "bg-rose-600"}`}>✅ 확인</button></div></>}
            {entryDialog.step === "RECHECKING" && <div className="py-10 text-center"><RefreshCw className="h-7 w-7 mx-auto animate-spin text-cyan-400" /><div className="mt-3 font-black">지금 가격을 다시 보고 있어요</div><div className="mt-1 text-xs text-slate-400">가격을 쫓아가서 사지 않도록 마지막으로 확인합니다.</div></div>}
            {(entryDialog.step === "WAIT" || entryDialog.step === "READY") && <div className="py-6 text-center">{entryDialog.step === "WAIT" ? <Clock3 className="h-8 w-8 mx-auto text-amber-400" /> : <ShieldCheck className="h-8 w-8 mx-auto text-emerald-400" />}<div className="mt-3 font-black">{entryDialog.step === "WAIT" ? "🟡 지금은 기다려요" : "🟢 조건 확인 완료"}</div><div className="mt-2 text-xs text-slate-400">{entryDialog.message || "기존 BuyMoney 주문 안전 확인 단계로 넘겼어요."}</div><button type="button" onClick={() => setEntryDialog(null)} className="mt-4 px-5 py-2 rounded-lg bg-slate-500/10 text-xs font-bold">닫기</button></div>}
          </div>
        </div>
      )}
