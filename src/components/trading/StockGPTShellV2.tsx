...
export default function StockGPTShellV2() {
  ...
  const loadSnapshot = async (quote: LiveMarketQuote, preferredName?: string) => {
    ...
    try {
      ...
      setSnapshotMessage(
        candles.length > 0 ? `검증된 실제 캔들 ${candles.length}개` : "현재가는 확인됐지만 과거 캔들은 NO_DATA",
      );
    } catch {
  ...
  const submitQuestion = (text = query) => {
    ...
    const shortRequest =
      lower.includes("숏") || lower.includes("하락") || lower.includes("약한") || lower.includes("short");
  ...
  const explanation = useMemo(() => {
    ...
    if (technicalScore != null && technicalScore > 0) {
      items.push(`기술 점수는 ${technicalScore}/100이에요. 수익 확률이 아니라 실제 차트 조건 점수예요.`);
    } else {
      items.push("실제 캔들이 부족하면 기술 점수도 NO_DATA로 둬요.");
    }
    return items;
    ...
    <div className="min-h-screen bg-[#050a12] text-slate-100" data-testid="stock-gpt-approved-layout">
      <div className="grid min-h-screen grid-cols-1 2xl:grid-cols-[230px_minmax(0,1fr)_300px]">
        <aside className="hidden border-r border-slate-800/80 bg-[#08111d] 2xl:flex 2xl:flex-col">
          <div className="border-b border-slate-800/80 px-5 py-5">
            <div className="flex items-center gap-2 text-xl font-black tracking-tight">
              <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-cyan-400/10 text-cyan-300 ring-1 ring-cyan-400/20">
          ...
          <div className="mt-auto space-y-3 p-4">
            <div className="rounded-xl border border-emerald-500/20 bg-emerald-950/10 p-3">
              <div className="flex items-center gap-2 text-xs font-black text-emerald-300">
                <Gift className="h-4 w-4" /> ZERO FAKE DATA
              </div>
            ...
            <div className="rounded-xl border border-violet-500/20 bg-violet-950/10 p-3">
              <div className="flex items-center gap-2 text-xs font-black text-violet-300">
                <Crown className="h-4 w-4" /> 실제 데이터 우선
              </div>
              <p className="mt-2 text-[11px] leading-5 text-slate-400">실시간 피드와 검증된 OHLCV만 화면에 사용합니다.</p>
            </div>
            <div className="flex items-center gap-2 border-t border-slate-800 pt-3 text-xs text-slate-400">
              <CircleUserRound className="h-5 w-5 text-slate-300" />
              <div>
                <div className="font-bold text-slate-300">사용자</div>
                <div className="text-[10px] text-slate-600">Stock GPT</div>
              </div>
            </div>
          </div>
        </aside>
        ...
        <main className="min-w-0 bg-[radial-gradient(circle_at_top,#10223d_0%,#07111f_34%,#050a12_76%)]">
          <div className="mx-auto w-full max-w-6xl px-4 py-5 sm:px-6 lg:px-7">
            <form
              ...
              className="sticky top-0 z-40 flex items-center gap-3 rounded-xl border border-slate-700/70 bg-[#0a1422]/95 p-2 shadow-[0_18px_60px_rgba(0,0,0,0.38)] backdrop-blur-xl"
            >
              ...
              <button
                type="submit"
                className="rounded-lg bg-cyan-400 p-3 text-slate-950 transition hover:bg-cyan-300"
                aria-label="질문 보내기"
              >
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {QUICK_PROMPTS.map((prompt) => (
                <button
                  ...
                  className="rounded-full border border-slate-800 bg-[#0a1422]/80 px-3 py-1.5 text-[11px] font-semibold text-slate-400 transition hover:border-cyan-400/40 hover:text-cyan-200"
                >
                  {prompt}
                </button>
              ))}
            ...
            {submittedQuery && (
              <div className="mt-6 flex items-start gap-3">
                <span className="mt-1 rounded-lg bg-cyan-400/10 p-2 text-cyan-300 ring-1 ring-cyan-400/20">
                  <Bot className="h-4 w-4" />
                </span>
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <strong className="text-sm">Stock GPT</strong>
                  ...
                  <p className="mt-2 text-sm leading-6 text-slate-300">“{submittedQuery}” 질문을 실제 확인 가능한 데이터만 사용해서 처리했어요.</p>
                </div>
              </div>
            )}
            ...
            {!submittedQuery && !selectedQuote && !pendingStock && (
              <section className="mt-10 rounded-2xl border border-slate-800 bg-[#091422]/70 p-8 text-center shadow-2xl">
                ...
                <p className="mx-auto mt-3 max-w-2xl text-sm leading-7 text-slate-400">질문하면 실제 시세, 실제 캔들, 실제 거래량을 확인한 뒤 차트와 쉬운 설명으로 보여줍니다.</p>
              </section>
            )}
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  {scanResults.map((quote, index) => (
                    <button
                      ...
                      type="button"
                      onClick={() => void loadSnapshot(quote)}
                      className="group rounded-xl border border-slate-800 bg-[#07101d] p-4 text-left transition hover:border-cyan-400/40"
                    >
                      <div className="flex justify-between gap-3">
                        <div>
                          <span className="text-[10px] font-black text-cyan-400">#{index + 1} {quote.market}</span>
                          <div className="mt-1 font-black text-white">{quote.name}</div>
                      ...
                      <div className="mt-4 grid grid-cols-2 gap-3 text-xs">
                        <div><div className="text-slate-500">현재가</div><strong>{formatPrice(quote.price, quote.market)}</strong></div>
                        <div><div className="text-slate-500">등락률</div><strong className={Number(quote.changeRate) >= 0 ? "text-rose-300" : "text-blue-300"}>{formatChange(quote.changeRate)}</strong></div>
                        <div><div className="text-slate-500">거래량</div><span>{formatCompact(quote.volume)}</span></div>
                        <div><div className="text-slate-500">출처</div><span className="text-emerald-300">{quote.source || "NO_DATA"}</span></div>
                      </div>
                    </button>
                  ))}
                </div>
            ...
            {submittedQuery && !selectedQuote && scanResults.length === 0 && snapshotMessage && (
              <div className="mt-5 rounded-xl border border-amber-500/20 bg-amber-950/10 p-4 text-sm text-amber-200">
                <div className="flex gap-2"><AlertTriangle className="h-4 w-4 shrink-0" /><span>{snapshotMessage}</span></div>
              </div>
            )}
              ...
              <section className="mt-5 overflow-hidden rounded-2xl border border-slate-800 bg-[#08111d]/96 shadow-[0_28px_80px_rgba(0,0,0,0.35)]">
                <div className="border-b border-slate-800 px-5 py-4">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        <h2 className="text-xl font-black">{chartName}</h2>
                        <span className="rounded-md bg-slate-900 px-2 py-1 text-xs font-mono text-slate-400">{selectedQuote.symbol}</span>
                        <span className="rounded-md border border-emerald-500/20 bg-emerald-500/10 px-2 py-1 text-[10px] font-black text-emerald-300">{selectedQuote.status}</span>
                      </div>
                      <div className="mt-3 flex flex-wrap items-end gap-3">
                        <span className="text-3xl font-black tracking-tight">{formatPrice(selectedQuote.price, selectedQuote.market)}</span>
                        <span className={Number(selectedQuote.changeRate) >= 0 ? "text-sm font-black text-rose-300" : "text-sm font-black text-blue-300"}>{formatChange(selectedQuote.changeRate)}</span>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div className="rounded-lg border border-slate-800 bg-[#050b14] px-3 py-2">
                        <div className="text-[10px] text-slate-500">차트 상태</div>
                        <strong className="text-cyan-300">{technicalScore != null ? chartState : "NO_DATA"}</strong>
                      </div>
                      <div className="rounded-lg border border-violet-500/20 bg-violet-950/10 px-3 py-2">
                        <div className="text-[10px] text-slate-500">Technical Score</div>
                        <strong className="text-violet-300">{technicalScore != null ? `${technicalScore}/100` : "NO_DATA"}</strong>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex gap-1 overflow-x-auto border-b border-slate-800 bg-[#07101b] px-4 text-[11px] font-bold text-slate-500">
                  {["차트", "기업 개요", "AI 분석 리포트", "재무 정보", "관련 뉴스", "토론 커뮤니티"].map((tab, index) => (
                    <button
                      key={tab}
                      type="button"
                      className={`whitespace-nowrap border-b-2 px-3 py-3 ${index === 0 ? "border-cyan-400 text-cyan-300" : "border-transparent text-slate-500"}`}
                      title={index === 0 ? "실시간 차트" : "기존 기능 연결 위치"}
                    >
                      {tab}
                    </button>
                  ))}
                ...
                <div className="relative bg-[#050a12] p-3 sm:p-4">
                  <div className="mb-3 flex flex-wrap items-center justify-between gap-2 px-1">
                    <div className="flex items-center gap-2 text-sm font-black"><LineChart className="h-4 w-4 text-cyan-300" />실시간 트레이딩 차트</div>
                    <span className="text-[11px] text-slate-500">{snapshotMessage}</span>
                  </div>
                  {chartPrice > 0 ? (
                ...
                <div className="grid gap-4 border-t border-slate-800 bg-[#08111d] p-4 xl:grid-cols-[minmax(0,1fr)_330px]">
                  <div className="space-y-3">
                    <div className="rounded-xl border border-cyan-500/20 bg-cyan-950/10 p-4">
                      <div className="mb-3 flex items-center gap-2 text-sm font-black"><Sparkles className="h-4 w-4 text-cyan-300" />초등학생도 이해하는 설명</div>
                      <div className="space-y-2 text-sm leading-6 text-slate-300">
                        {explanation.map((line, index) => <p key={`${index}-${line}`}>{index + 1}️⃣ {line}</p>)}
                      </div>
                    </div>
                    <div className="rounded-xl border border-sky-500/20 bg-sky-950/10 p-4">
                      <div className="text-xs font-black text-sky-300">한 줄 요약</div>
                      <p className="mt-2 text-sm leading-6 text-slate-300">{oneLineSummary}</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 xl:grid-cols-1">
                    <button
                      ...
                      className="rounded-xl border border-emerald-500/35 bg-emerald-500/10 p-4 text-left transition hover:bg-emerald-500/16"
                    >
                      <ArrowUpRight className="h-5 w-5 text-emerald-300" />
                      <div className="mt-2 text-base font-black text-emerald-200">LONG 검토</div>
                      <div className="mt-1 text-xs text-emerald-300/70">주문 전 확인만</div>
                    </button>
                    <button
                      type="button"
                      onClick={() => setReviewSide("SHORT")}
                      className="rounded-xl border border-rose-500/30 bg-rose-500/10 p-4 text-left transition hover:bg-rose-500/16"
                    >
                      <ArrowDownRight className="h-5 w-5 text-rose-300" />
                      <div className="mt-2 text-base font-black text-rose-200">SHORT 검토</div>
        ...
        <aside className="hidden border-l border-slate-800/80 bg-[#08111d] 2xl:block">
          <div className="sticky top-0 space-y-3 p-4">
            <div className="rounded-xl border border-slate-800 bg-[#0a1422] p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="text-xs font-black text-slate-300">{kstDate}</div>
                  <div className="mt-1 text-lg font-black tracking-tight text-white">{kstTime}</div>
                </div>
                <span className="rounded-full border border-emerald-500/20 bg-emerald-500/10 px-2 py-1 text-[10px] font-black text-emerald-300">● 실시간</span>
              </div>
            </div>

            <div className="rounded-xl border border-slate-800 bg-[#0a1422] p-4">
              <div className="flex items-center gap-2 text-sm font-black"><Activity className="h-4 w-4 text-cyan-300" />시장 상태</div>
              <div className="mt-3 space-y-2">
                {marketSummaries.map((row) => (
                  <div key={row.market} className="rounded-lg border border-slate-800 bg-[#07101b] p-3">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-xs font-black text-slate-300">{row.market} LIVE 평균</span>
                      <span className={row.averageChange == null ? "text-xs font-black text-slate-500" : Number(row.averageChange) >= 0 ? "text-xs font-black text-rose-300" : "text-xs font-black text-blue-300"}>{formatChange(row.averageChange)}</span>
                    </div>
                    <div className="mt-2 text-[10px] text-slate-500">검증 LIVE 종목 {row.liveCount}개</div>
                  </div>
                ))}
              </div>
              <p className="mt-3 text-[10px] leading-5 text-slate-600">시장지수 자체가 아니라 현재 검증된 LIVE 종목의 평균 등락률입니다. 지수 데이터가 없으면 임의 지수를 만들지 않습니다.</p>
            </div>

            <div className="rounded-xl border border-slate-800 bg-[#0a1422] p-4">
              <div className="flex items-center gap-2 text-sm font-black"><TrendingUp className="h-4 w-4 text-emerald-300" />강한 섹터/테마 TOP 5</div>
              <div className="mt-3 space-y-1.5">
                {strongThemes.length > 0 ? (
                  strongThemes.map((row, index) => (
                    <div key={row.theme} className="flex items-center justify-between gap-3 rounded-lg px-2 py-2 hover:bg-slate-900/70">
                      <span className="truncate text-xs font-bold text-slate-300">{index + 1}. {row.theme}</span>
                      <span className={Number(row.averageChange) >= 0 ? "text-xs font-mono font-black text-rose-300" : "text-xs font-mono font-black text-blue-300"}>{formatChange(row.averageChange)}</span>
                    </div>
                  ))
                ) : (
            ...
            <div className="rounded-xl border border-slate-800 bg-[#0a1422] p-4">
              <div className="flex items-center gap-2 text-sm font-black"><TrendingUp className="h-4 w-4 text-rose-300" />실시간 급상승 종목</div>
              <div className="mt-3 space-y-1">
                {strongest.length > 0 ? (
                  strongest.map((quote, index) => (
                    <button
                      key={`side-strong-${quote.market}-${quote.symbol}`}
                      type="button"
                      onClick={() => void loadSnapshot(quote)}
                      className="flex w-full items-center justify-between rounded-lg px-2 py-2 text-left transition hover:bg-slate-900"
                    >
                      <span className="truncate text-xs font-bold text-slate-300">{index + 1}. {quote.name}</span>
                      <span className="text-xs font-mono font-black text-rose-300">{formatChange(quote.changeRate)}</span>
                    </button>
                  ))
                ) : (
                  <div className="py-4 text-center text-xs text-slate-600">NO_DATA</div>
            ...
            <div className="rounded-xl border border-slate-800 bg-[#0a1422] p-4">
              <div className="flex items-center gap-2 text-sm font-black"><AlertTriangle className="h-4 w-4 text-amber-300" />위험 신호 종목</div>
              <div className="mt-3 space-y-1">
                {weakest.length > 0 ? (
                  weakest.map((quote) => (
                    <button
                      key={`side-risk-${quote.market}-${quote.symbol}`}
                      type="button"
                      onClick={() => void loadSnapshot(quote)}
                      className="flex w-full items-center justify-between rounded-lg px-2 py-2 text-left transition hover:bg-slate-900"
                    >
                      <span className="truncate text-xs font-bold text-slate-300">{quote.name}</span>
                      <span className="text-xs font-mono font-black text-blue-300">{formatChange(quote.changeRate)}</span>
                    </button>
                  ))
                ) : (
                  <div className="py-4 text-center text-xs text-slate-600">NO_DATA</div>
              ...
              <div className="mt-3 flex items-center justify-between rounded-lg bg-[#07101b] px-3 py-2 text-[11px]">
                <span className="text-slate-500">STALE / 미검증</span>
      ...
      {reviewSide && selectedQuote && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-slate-700 bg-[#091321] p-5 shadow-2xl">
            <div className="flex justify-between gap-4">
              <div>
                <div className={reviewSide === "LONG" ? "text-xs font-black text-emerald-300" : "text-xs font-black text-rose-300"}>{reviewSide} REVIEW</div>
                <h3 className="mt-1 text-lg font-black">{chartName} 거래 검토</h3>
              </div>
              <button type="button" onClick={() => setReviewSide(null)} className="rounded-lg p-1 text-slate-500 hover:text-white"><X className="h-5 w-5" /></button>
            </div>
            <div className="mt-5 space-y-2 rounded-xl border border-slate-800 bg-slate-950/70 p-4 text-sm">
              <div className="flex justify-between"><span className="text-slate-500">현재가</span><strong>{formatPrice(selectedQuote.price, selectedQuote.market)}</strong></div>
              <div className="flex justify-between"><span className="text-slate-500">데이터</span><strong className="text-emerald-300">{selectedQuote.status} / {selectedQuote.source || "NO_DATA"}</strong></div>
              <div className="flex justify-between"><span className="text-slate-500">차트 상태</span><strong>{technicalScore != null ? chartState : "NO_DATA"}</strong></div>
            </div>
            <div className="mt-4 rounded-xl border border-amber-500/20 bg-amber-950/10 p-3 text-xs leading-5 text-amber-200">검토 단계만 열립니다. 수량·금액·손절·목표가가 실제 주문 승인 게이트에서 검증되기 전에는 주문을 전송하지 않습니다.</div>
            <button type="button" onClick={() => setReviewSide(null)} className="mt-4 w-full rounded-lg bg-slate-800 px-4 py-3 text-sm font-black hover:bg-slate-700">확인하고 닫기</button>
          </div>
        </div>
      )}
    </div>
