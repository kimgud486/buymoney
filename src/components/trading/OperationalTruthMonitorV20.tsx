...
export const OperationalTruthMonitorV20: React.FC = () => {
  ...
  const selectedSymbol = typeof rawSelectedSymbol === "string"
    ? rawSelectedSymbol
    : String(rawSelectedSymbol?.symbol || "005930");
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const response = await fetch(`/api/broker/v21/runtime?symbol=${encodeURIComponent(selectedSymbol)}`, {
  ...
  const view = useMemo(() => {
    ...
    return buildOperationalTruthViewModelV20({
      ...
      brokerDataStatus: String(kis?.dataStatus || (isKoreaSymbol ? "NO_DATA" : "NO_DATA")),
  ...
  }, [engineStatus, kis, isKoreaSymbol]);

  const gateTone = view.gateState === "READY"
    ? "bg-emerald-100 text-emerald-800 border-emerald-300"
    : view.gateState === "TEST_ONLY"
  ...
  return (
    <section
      ...
      className="mx-auto w-full max-w-[1920px] px-2 pt-2"
      aria-label="실거래 운영 진실 모니터"
    >
      <div className="rounded-2xl border border-zinc-200 bg-white p-3 shadow-sm">
        <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-2">
            <span className="rounded-xl bg-slate-950 p-2 text-cyan-300"><Radio className="h-4 w-4" /></span>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h2 className="text-sm font-black text-zinc-900">운영 진실 모니터 V20</h2>
                <span data-testid="operational-gate-state" className={`rounded-full border px-2 py-0.5 text-[10px] font-black ${gateTone}`}>
                  {view.gateLabel}
                </span>
              </div>
              <p className="text-[10px] text-zinc-500">주문상태 · Risk Gate · KIS 실계좌 truth · 오늘 손익을 같은 화면에서 확인</p>
            </div>
          </div>
          ...
          <div className="text-[10px] text-zinc-500">
            {isKoreaSymbol
              ? fetchError
                ? `KIS 조회 오류: ${fetchError}`
                : lastFetchAt
                  ? `KIS 확인 ${new Date(lastFetchAt).toLocaleTimeString("ko-KR")}`
                  : "KIS 확인 중"
              : "해외/업비트 선택 중: KIS 국내 실계좌 proof는 적용 안 됨"}
          </div>
        </div>
        ...
        {(view.blockers.length > 0 || fetchError) && (
          <details className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2">
            <summary className="cursor-pointer text-[11px] font-black text-amber-900 flex items-center gap-1">
              <AlertTriangle className="h-3.5 w-3.5" /> 차단/주의 사유 보기 ({view.blockers.length + (fetchError ? 1 : 0)})
            </summary>
            <div className="mt-2 flex flex-wrap gap-1.5">
              {view.blockers.slice(0, 12).map((item) => (
                <span key={item} className="rounded bg-white px-2 py-1 text-[10px] font-bold text-amber-900 border border-amber-200">{item}</span>
              ))}
              {fetchError && <span className="rounded bg-white px-2 py-1 text-[10px] font-bold text-rose-700 border border-rose-200">{fetchError}</span>}
            </div>
          </details>
        )}
      </div>
...
const TruthCard: React.FC<{ icon: React.ReactNode; label: string; value: string; sub: string }> = ({ icon, label, value, sub }) => (
  <div className="min-w-0 rounded-xl border border-zinc-200 bg-zinc-50 p-2.5">
    <div className="flex items-center gap-1 text-[10px] font-bold text-zinc-500">{icon}<span>{label}</span></div>
    <div className="mt-1 truncate text-xs font-black text-zinc-900">{value}</div>
    <div className="mt-0.5 truncate text-[9px] text-zinc-500">{sub}</div>
  </div>
);
