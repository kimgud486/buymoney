...
function MainLayout() {
  ...
  // Preserve the existing dashboard button and make the technical-chart button
  // act as the large-chart entry point. "지표 설정" remains a dropdown only.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const closeOnEscape = (event: KeyboardEvent) => {
      if (event.key === "Escape" && !document.fullscreenElement) setIsMainChartExpanded(false);
    };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans relative">
      ...
      {isMainChartExpanded && (
        <div className="fixed inset-0 z-[250] bg-slate-950/95 backdrop-blur-md p-1.5 sm:p-3">
          <div
            ref={expandedChartShellRef}
            className="h-full w-full min-h-0 overflow-hidden rounded-2xl border border-cyan-500/40 bg-slate-950 shadow-2xl flex flex-col"
          >
            <div className="shrink-0 flex flex-wrap items-center justify-between gap-2 border-b border-slate-800 bg-slate-950/95 px-3 py-2.5 sm:px-4">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <strong className="text-sm sm:text-base text-white">📈 그래프 지표 확대뷰</strong>
                ...
                <div className="mt-1 flex items-center gap-1 text-[11px] text-slate-400">
                  ...
                  마우스 휠/두 손가락으로 확대·축소 · 드래그로 좌우 이동
                </div>
              </div>
            ...
            <div className="flex-1 min-h-0 p-1.5 sm:p-2">
              {isExpandedChartLoading ? (
                <div className="h-full min-h-[65vh] flex items-center justify-center text-sm text-cyan-300">
                  실시간 차트를 불러오는 중입니다...
                </div>
              ) : expandedChart.candles.length === 0 || expandedChart.price <= 0 ? (
                <div className="h-full min-h-[65vh] flex flex-col items-center justify-center gap-2 text-center text-slate-400">
                  <strong className="text-white">실시간 차트 데이터가 아직 없습니다.</strong>
                  <span className="text-xs">가짜 그래프는 표시하지 않고 검증된 캔들이 들어오면 확대 차트를 표시합니다.</span>
                </div>
              ) : (
                <ErrorBoundary>
                  <RealTimeTradingViewChart
                    key={`expanded-${expandedChart.symbol}`}
                    ...
                    timeframe="1m"
                    isWhiteTheme={false}
                    className="h-full min-h-[72vh] w-full border-0 shadow-none"
                  />
                </ErrorBoundary>
              )}
            </div>
