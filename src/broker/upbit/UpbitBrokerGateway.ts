...
export class UpbitBrokerGateway implements BrokerGateway {
  ...
  public async submitBuy(request: OrderRequest): Promise<BrokerOrderResult> {
    if (!this.healthy) {
      return {
        success: false,
        orderId: "",
        message: "Upbit Broker gateway disconnected or unhealthy"
      };
    }

    const orderId = `UPBIT_ORD_${Date.now()}_${Math.floor(Math.random() * 1000)}`;
    ...
    return {
      success: true,
      orderId,
      message: "Upbit BUY Order Acknowledged",
  ...
  public async submitSell(request: OrderRequest): Promise<BrokerOrderResult> {
    ...
    return {
      success: true,
      orderId,
      message: "Upbit SELL Order Acknowledged",
      executionEvent: event
    };
  }
