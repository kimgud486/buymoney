  ...
  public static scanMarket(
    ...
    for (const stock of stocks) {
      ...
      results.push({
        ...
        metrics: {
          rvol, vwap, ema9: snapshot.ema9, ema20: snapshot.ema20, ema50: snapshot.ema50, rsi14: rsi, atr14: atr,
          rs15m: null, breakoutConfirmed: vwap != null ? price >= vwap : null, chaseRisk, exhaustionRisk,
...
export async function scanGlobalRealtimeHotListV192(options?: {
  ...
  if (normalizedMarket === "ALL" || normalizedMarket === "KOREA") {
    ...
    diagnostics.KOREA = { universe: krStocks.length, liveQuoteReady: krRes.liveQuoteReady, candle15mReady: krRes.candle15mReady, passed: krRes.items.length };
  }
  if (normalizedMarket === "ALL" || normalizedMarket === "US") {
    ...
    diagnostics.US = { universe: usStocks.length, liveQuoteReady: usRes.liveQuoteReady, candle15mReady: usRes.candle15mReady, passed: usRes.items.length };
  }
  if (normalizedMarket === "ALL" || normalizedMarket === "UPBIT") {
    ...
    diagnostics.UPBIT = { universe: upbitStocks.length, liveQuoteReady: upbitRes.liveQuoteReady, candle15mReady: upbitRes.candle15mReady, passed: upbitRes.items.length };
  }
